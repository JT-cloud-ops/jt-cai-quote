...
export const BookletQuotationRows: React.FC<Props> = ({ job, isDepartment }) => {
  ...
  return <React.Fragment>
    <tr><td className="quote-cell-center-wrap" style={{ fontWeight: 'bold' }}>{job.jobName}</td><td className="quote-cell-center-wrap">{job.jobSheetSize}</td><td className="quote-cell-center-wrap">&nbsp;</td><td className="quote-cell-center-wrap">&nbsp;</td><td className="quote-cell-center-wrap multi-line">{job.bindingMethod}</td><td rowSpan={totalRowsForJob} className="quote-cell-center-wrap">{job.quantity ? `${job.quantity}${job.unit}` : ''}</td><td rowSpan={totalRowsForJob} className="quote-cell-center-wrap">{job.unitPrice ? formatCurrency(parseFloat(job.unitPrice) || 0) : ''}</td><td rowSpan={totalRowsForJob} className="quote-cell-center-wrap">{amount > 0 ? <>{formatCurrency(amount)}<span style={{ fontSize: 'calc(8pt * var(--layout-scale))', marginLeft: '2pt', display: 'inline-block' }}>(未稅)</span></> : ''}</td></tr>
